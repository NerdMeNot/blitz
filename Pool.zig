...
const AtomicCounters = struct {
    ...
    /// AcqRel is sufficient - this is bookkeeping, not part of the JEC protocol.
    /// The SeqCst operations in incrementJecIfSleepy/tryAddSleepingThread
    /// provide the necessary ordering for the sleep protocol.
    fn addInactiveThread(self: *AtomicCounters) void {
        _ = self.value.fetchAdd(ONE_INACTIVE, .acq_rel);
    }
    ...
    fn subInactiveThread(self: *AtomicCounters) u16 {
        ...
        return @min(sleeping, 2); // O(log N) cascade with branching factor 2
    }
...
const WorkerSleepState = struct {
    ...
    /// Atomic to allow lock-free pre-check in wakeSpecificThread.
    /// Writes still happen under mutex; the atomic enables a fast
    /// "definitely not blocked" check without taking the lock.
    is_blocked: std.atomic.Value(bool),
    // Padding to ensure each WorkerSleepState is on its own cache line
    _padding: [std.atomic.cache_line - (@sizeOf(std.Thread.Mutex) + @sizeOf(std.Thread.Condition) + @sizeOf(std.atomic.Value(bool))) % std.atomic.cache_line]u8 = undefined,

    fn init() WorkerSleepState {
        return .{
            .mutex = .{},
            .condvar = .{},
            .is_blocked = std.atomic.Value(bool).init(false),
            ._padding = undefined,
        };
    }
};
...
pub const Sleep = struct {
    ...
    fn sleep(
        ...
        // Step 4: Check injected jobs.
        // The SeqCst CAS in tryAddSleepingThread above acts as a full
        // memory barrier on real hardware (x86 LOCK prefix, ARM DMB),
        // ensuring the injected job store is visible if it preceded our
        // sleeping registration. No separate fence needed.
        ...
        state.is_blocked.store(true, .release);
        while (state.is_blocked.load(.acquire) and !pool.stopping.load(.acquire)) {
    ...
    pub fn newInjectedJobs(self: *Sleep, num_jobs: u32, queue_was_empty: bool) void {
        // SeqCst barrier ensures the injected job store (AcqRel CAS on
        // injected_jobs_head) is visible before we read sleeping count.
        // Pairs with the SeqCst CAS in tryAddSleepingThread on the sleep path.
        // Uses counters instead of a separate sentinel to avoid bouncing
        // an extra cache line.
        _ = self.counters.value.fetchAdd(0, .seq_cst);
    ...
    fn newJobs(self: *Sleep, num_jobs: u32, queue_was_empty: bool) void {
        ...
        self.wakeThreads(num_to_wake, null);
    }

    /// Wake up to `count` sleeping threads.
    /// `preferred_start` provides NUMA-aware wake ordering: on systems where
    /// adjacent worker IDs map to the same socket/node, starting from the
    /// caller's index wakes nearby (cache-warm) workers first.
    pub fn wakeThreads(self: *Sleep, count: u32, preferred_start: ?usize) void {
        const n = self.worker_sleep_states.len;
        if (n == 0 or count == 0) return;
        const start = preferred_start orelse 0;
        var woken: u32 = 0;
        for (0..n) |i| {
            if (woken >= count) break;
            const idx = (start + i) % n;
            if (self.wakeSpecificThread(&self.worker_sleep_states[idx])) {
                woken += 1;
            }
        }
    }
    ...
    fn wakeSpecificThread(self: *Sleep, state: *WorkerSleepState) bool {
        // Fast atomic pre-check: if not blocked, skip the mutex entirely.
        // This avoids lock contention for workers still in the yield/spin phase.
        if (!state.is_blocked.load(.acquire)) return false;

        state.mutex.lock();
        defer state.mutex.unlock();

        // Re-check under mutex (could have changed between pre-check and lock)
        if (state.is_blocked.load(.acquire)) {
            state.is_blocked.store(false, .release);
    ...
    pub fn wakeAll(self: *Sleep) void {
        for (self.worker_sleep_states) |*state| {
            _ = self.wakeSpecificThread(state);
        }
    }
};
    ...
    while (!pool.stopping.load(.acquire)) {
        ...
        while (!pool.stopping.load(.acquire)) {
            if (pool.findWork(worker)) |job| {
                ...
                executeJob(worker, job);
                pool.sleep.wakeThreads(to_wake, worker_id);
                break;
            }
