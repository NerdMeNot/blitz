...
slug: v2.0.0-zig0.15.3/usage/fork-join
---
...
* **[Error Handling](/v2.0.0-zig0.15.3/usage/error-handling/)** — Use `tryJoin()` when your tasks return error unions. All tasks complete before errors propagate.
* **[Scope & Broadcast](/v2.0.0-zig0.15.3/usage/scope-broadcast/)** — Use `scope()` for dynamic task spawning (up to 64 tasks) and `broadcast()` to run on all workers.
* **[Choosing the Right API](/v2.0.0-zig0.15.3/guides/choosing-api/)** — Decision guide for when to use `join()` vs `parallelFor` vs iterators.
