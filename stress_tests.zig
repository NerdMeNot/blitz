    ...
    for (0..NUM_CONCURRENT) |i| {
        threads[i] = try std.Thread.spawn(.{}, struct {
            fn run(arr: []i64, allocator: std.mem.Allocator) void {
                mod.parallelSort(i64, arr, allocator) catch {};
            }
...
test "parallelSum - 10M elements" {
    ...
    const result = mod.parallelSum(i64, data);
...
test "parallelSort - 10M elements" {
    ...
    try mod.parallelSort(i64, data, std.testing.allocator);
...
test "parallelMin/Max - 10M elements" {
    ...
    const min_result = mod.parallelMin(i64, data);
    const max_result = mod.parallelMax(i64, data);
...
test "parallelFind - 10M elements" {
    ...
    const result = mod.parallelFindAny(i64, data, struct {
        fn pred(x: i64) bool {
            return x == 42;
        }
    }.pred);
...
test "parallelFor - 10M elements" {
    ...
    mod.parallelFor(SIZE, Context, .{ .data = data }, struct {
...
test "iter - 10M elements reduce" {
    ...
    const result = mod.iter(i64, data).sum();
